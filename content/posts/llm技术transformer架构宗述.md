...
![词嵌入技术示意](llm技术transformer架构宗述/image-20251015205211766.png)
...
![RNN机构示意](llm技术transformer架构宗述/image-20251015205426121.png)
...
![LSMT结构示意](llm技术transformer架构宗述/image-20251015205527512.png)
...
![img](llm技术transformer架构宗述/1533981-20250718224012244-220282788.png)
...
![img](llm技术transformer架构宗述/v2-613bffcc75eeae483f3d3de6dc1a1b54_r.jpg)
...
> 使用机器翻译将一句西语翻译为应用：
>
> ![ML翻译应用示意](llm技术transformer架构宗述/image.png)
...
![每个单词都嵌入到一个大小为512的向量中](llm技术transformer架构宗述/image-5.png)
...
![it对于输入内容中所有单词的关系](llm技术transformer架构宗述/image-8-1760534908273-1.png)
...
![x1乘以WQ权重矩阵可得出q1，即与该词相关的“查询”向量。最终会为输入句子中的每个词创建一个“查询”、“键”和一个“值”投影](llm技术transformer架构宗述/image-9-1760535614833-4.png)
...
![Softmax归一化处理计算出权重值](llm技术transformer架构宗述/image-20251016000021074.png)
...
![当嵌入向量的长度是4的时候，位置编码的长度也是4](llm技术transformer架构宗述/image-22-1760545817067-38.png)
...
> - **具体步骤是**：
>   - **保留原版**：你把做好的“版本A”完整地留在盘子里。`（这就是 x，输入）`
>   - **制作“改进包”**：你另外拿个小碗，调一点点更鲜的酱油汁，或者切一点葱花。`（这就是 F(x)，要学习的“残差”或“变化”）`
>   - **合并**：最后，你把小碗里的“改进包”浇到“版本A”上。`（这就是 x + F(x)，输出 = 输入 + 变化）`
...
![ 2 个堆叠的编码器和解码器组成的 Transformer的数据流](llm技术transformer架构宗述/image-27-1760546832738-50.png)
...
>  - **询问编码器**：接下来，撰写员要写下一个词了。他思考：“基于我目前写的 `‘I love’`，我应该去法语文档的哪个部分寻找最重要的信息来确定下一个词？”他拿着当前的状态（对应一个 Query，Q），去查询编码器提供的那份索引K。通过比对，他发现他的Q与编码器K中代表 `"beau"`（美丽）和 `"jardin"`（花园）的键最匹配。特别是 `"jardin"`（花园）匹配度最高。于是，他将**注意力集中**到编码器信息V中与 `"jardin"`相关的实质内容上。
