...
![UV坐标与展开](games101纹理映射与着色/image-20250722165200809.png)
...
![三角形重心计算](games101纹理映射与着色/%E5%9B%BE%E7%89%871.png)
...
![重心插值](games101纹理映射与着色/image-20251128200825154.png)
...
![纹理放大模糊问题](games101纹理映射与着色/image-20251128201234821.png)

### 最近邻采样（Nearest Neighbor Sampling）

![最近邻采样](games101纹理映射与着色/image-20251128201817529.png)
...
![双线性插值](games101纹理映射与着色/%E5%9B%BE%E7%89%872.png)
...
![纹理缩小失真与摩尔纹](games101纹理映射与着色/image-20251129010504959.png)
...
![超采样](games101纹理映射与着色/image-20251129010822835.png)

以512个小像素为例，得到的结果如下图

![超采样结果](games101纹理映射与着色/image-20251129010840417.png)
...
![Mipmap](games101纹理映射与着色/image-20251129011638283.png)

> [!IMPORTANT]
>
> 因为生成了多个较小的图像，需要额外储存生成的小图像，Mipmap方法算是一个经典的空间换时间策略。在某些教材表述中，该方法也被称为“纹理金字塔”(Texture Pyramid)。

![纹理金字塔](games101纹理映射与着色/image-20251129012121646.png)
...
![深度远近区分](games101纹理映射与着色/image-20251129013145735.png)

这时候我们再延续之前的插值思想，把离散变为连续，在层与层之间进行插值，这个方法就是**三线性插值**。

![三线性插值](games101纹理映射与着色/image-20251129013405597.png)
...
![多Mipmap平滑过渡](games101纹理映射与着色/image-20251129013517085.png)

在经历了插值后，可以看到，衔接处的割裂情况少了很多，过渡更加平滑。

### 各向异性过滤Anisotropic Filtering

![各向异性](games101纹理映射与着色/image-20251129014324864.png)
...
![不同区域的实际映射的真实纹理](games101纹理映射与着色/image-20251129014903467.png)
...
![Ripmap](games101纹理映射与着色/image-20251129020204129.png)
...
![EWS过滤](games101纹理映射与着色/image-20251129020550873.png)
...
![环境光贴图](games101纹理映射与着色/image-20251129201700456.png)
...
![环境光照](games101纹理映射与着色/image-20251129202107492.png)
...
![橘子表面](games101纹理映射与着色/image-20251129202841641.png)

这种技术广泛应用于游戏和电影中，以减少模型的多边形数量，同时增加视觉细节。

![凹凸纹理](games101纹理映射与着色/image-20251129203352163.png)
...
![法线计算示意](games101纹理映射与着色/%E5%9B%BE%E7%89%873.png)
...
![位移映射](games101纹理映射与着色/image-20251129203114986.png)

此外对于复杂模型的环境光照，也可以预先计算好存储在纹理中：

![带有环境光照的人表面纹理](games101纹理映射与着色/image-20251129204156754.png)

纹理还不止于3D物体表面，也可以用于存储空间结构，比如3D模型内部的密度，医学中使用3D纹理直接进行体积渲染

![3D内部纹理](games101纹理映射与着色/image-20251129204325334.png)