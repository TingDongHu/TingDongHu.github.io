...
![双向路径追踪示意](games101高级光线传播与复杂外观建模/image-20251216173414432.png)

**原理**：既然只从摄像机发射光线（Path Tracing）很难找到光源，那不如**同时从摄像机和光源**各发射一条半路径，然后把中间的端点连起来 。

**适用场景**：

![双向路径追踪](games101高级光线传播与复杂外观建模/image-20251216173454445.png)
...
![MLT光线传播方法](games101高级光线传播与复杂外观建模/image-20251216173630204.png)
...
![BDRT和MLT比对](games101高级光线传播与复杂外观建模/image-20251216174125805.png)
...
![散焦现象](games101高级光线传播与复杂外观建模/image-20251216175028771.png)

这是一种专门用来做**焦散 (Caustics)** 的神技（例如阳光透过装水的玻璃杯在桌上形成的亮斑）。它是一种**有偏但一致 (Biased but Consistent)** 的两阶段方法 。

**步骤通解**：

![光子映射示意](games101高级光线传播与复杂外观建模/image-20251216175445657.png)
...
![光子映射效果](games101高级光线传播与复杂外观建模/image-20251216175751655.png)

**直观表现**：N 取小了，会有噪点；N 取大了，画面会变糊（Bias 变大）。

 ### 顶点连接与合并 (VCM)与实时辐射度 (Instant Radiosity)

![VCM示意](games101高级光线传播与复杂外观建模/image-20251216175930636.png)

**VCM**：把 BDPT 和光子映射结合起来，集百家之长 。

![VCM示意图](games101高级光线传播与复杂外观建模/image-20251216175954589.png)
...
![参与介质物体](games101高级光线传播与复杂外观建模/image-20251216180227843.png)

![微小粒子的吸收和散射](games101高级光线传播与复杂外观建模/image-20251216180331336.png)
...
![光线在介质中的传播](games101高级光线传播与复杂外观建模/image-20251216180753202.png)
...
![image-20251216180829631](games101高级光线传播与复杂外观建模/image-20251216180829631.png)
...
![塑料感毛发模型](games101高级光线传播与复杂外观建模/image-20251216181214997.png)
...
![角蛋白毛发模型](games101高级光线传播与复杂外观建模/image-20251216181852719.png)
...
![疯狂动物城中的毛发渲染，看起来每个动物都和毛娃娃一样](games101高级光线传播与复杂外观建模/image-20251216181957137.png)

#### Double Cylinder 模型 (动物专用)

![动物毛发模型](games101高级光线传播与复杂外观建模/image-20251216182721909.png)

**背景：** 闫令琪老师团队发现，用 Marschner 模型渲染动物（如狼）不管怎么调参数都不像。

研究生物蛋白结构发现： 动物毛发比人发粗，中间有一根充满散射颗粒的**髓质 (Medulla)**。

![髓质毛发模型原理](games101高级光线传播与复杂外观建模/image-20251216182808241.png)

其团队做了如下改进： 在 Marschner 的玻璃圆柱里，再套一个小圆柱（髓质），专门负责散射光线 (S 分量)。这让动物毛发看起来更“厚实”且有杂色。

![髓质毛发模型绘制](games101高级光线传播与复杂外观建模/image-20251216183035761.png)

该技术被用在了电影金刚和狮子王中，并获得了2018和2019奥斯卡最佳视觉提名

![金刚和狮子王中的毛发渲染，非常逼真](games101高级光线传播与复杂外观建模/image-20251216183328826.png)
...
![颗粒才材质](games101高级光线传播与复杂外观建模/image-20251216183726951.png)
...
![次表面散射](games101高级光线传播与复杂外观建模/image-20251216184545833.png)
...
![此表面散射工程解法](games101高级光线传播与复杂外观建模/image-20251216184306059.png)

为了模拟光在表面下的扩散光斑，他在物体表面上方放一个**正光源**，在表面下方放一个**负光源**。这两个虚拟光源的共同作用，恰好能形成一个类似于真实次表面散射的“衰减光斑”。把复杂的物理积分，简化成了简单的数学公式。这让实时渲染皮肤成为可能。

![CG中的晶莹剔透的人脸](games101高级光线传播与复杂外观建模/image-20251216184622860.png)
...
![布料的几种渲染逻辑](games101高级光线传播与复杂外观建模/image-20251216184951472.png)

**作为表面 (As Surface)**

远距离观察的情况下，或者像丝绸这样编织极密的布料。可以通过直接修改 BRDF进行模拟。

![布料表面模型](games101高级光线传播与复杂外观建模/image-20251216185549836.png)
...
![布料介质模型](games101高级光线传播与复杂外观建模/image-20251216185713793.png)
...
![布料纤维模型](games101高级光线传播与复杂外观建模/image-20251216185815788.png)
...
![P-NDF绘制](games101高级光线传播与复杂外观建模/image-20251216191341093.png)

**视觉效果**：实现了**Glints (闪烁)**。随着视角微小的移动，高光会剧烈跳动，这完美复现了远处看海面波光粼粼，或者在阳光下看金属漆面的那种颗粒感。

#### 波动光学 (Wave Optics)

![波动光学与干涉衍射](games101高级光线传播与复杂外观建模/image-20251216190609563.png)
...
![PCG纹理应用](games101高级光线传播与复杂外观建模/image-20251216191007918.png)
