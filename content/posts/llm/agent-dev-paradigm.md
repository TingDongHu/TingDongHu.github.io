...
math: true
description: "深入解析LLM Agent三大经典范式：ReAct、Plan-and-Execute与Reflection。从设计思想到代码实现，理解边想边做、谋定而后动、三省吾身三种思维方式在AI系统中的应用。"
---
...
> **延伸阅读**：
> - [LangChain: Getting Started with Agents](https://python.langchain.com/docs/tutorials/agents/) - 主流Agent框架的实现细节
> - [AutoGPT: Autonomous GPT-4 Agent](https://github.com/Significant-Gravitas/AutoGPT) - 首个自主Agent的探索
> - [BabyAGI: Task-driven Autonomous Agent](https://github.com/yoheinakajima/babyagi) - 任务驱动的自主Agent实现
...
> **延伸阅读**：
> - [ReAct: Synergizing Reasoning and Acting in Language Models](https://arxiv.org/abs/2210.03629) - ReAct原始论文
> - [Chain of Thought Prompting Elicits Reasoning in Large Language Models](https://arxiv.org/abs/2201.11903) - 思维链基础研究
...
想象一个项目经理解决复杂问题的过程：
1. **规划阶段**：项目经理（Planner）先分析任务需求，制定详细的执行计划，确定每个步骤的顺序和依赖关系
2. **执行阶段**：工程师（Executor）按照计划逐个实施，每完成一步汇报进度
3. **监控阶段**：项目经理监控执行进度，遇到偏差时调整计划（Re-planning）
...
![Plan-and-Execute架构](https://pub-500a3a9e99b44ef29efa70fd87011d69.r2.dev/agent/llm应用开发原理agent经典开发范式/02-framework-plan-execute.png)
...
> **延伸阅读**：
>
> - [Plan-and-Solve: Breaking Down Complex Reasoning Tasks](https://arxiv.org/abs/2210.03106) - 规划与执行范式研究
> - [HuggingGPT: Solving AI Tasks with ChatGPT and its Friends](https://arxiv.org/abs/2303.17554) - 规划驱动的多模型协作
> - [TaskMatrix: Compositional Generalization via Task Matrix](https://arxiv.org/abs/2306.07133) - 任务规划与组合泛化
...
![Reflection反思机制](https://pub-500a3a9e99b44ef29efa70fd87011d69.r2.dev/agent/llm应用开发原理agent经典开发范式/03-flowchart-reflection.png)

想象一个写作者打磨作品的过程：
1. **初稿**：快速写出第一版内容
2. **自我审阅**：从读者角度审视，发现问题点
3. **批判反馈**：指出逻辑漏洞、表达不当、事实错误
4. **修改完善**：基于反馈进行针对性修改
5. **迭代优化**：重复审阅-修改循环，直到满意
...
> **延伸阅读**：
> - [Reflexion: Language Agents with Verbal Reinforcement Learning](https://arxiv.org/abs/2303.11366) - 反思增强Agent研究
> - [Self-Refine: Language Models Can Self-Correct](https://arxiv.org/abs/2304.05992) - 自修正能力研究
> - [CRITIC: Large Language Models Can Self-Correct with Tool-Interactive Critiquing](https://arxiv.org/abs/2310.08106) - 工具辅助反思机制

## 如何选取合适的范式

![Agent三大范式对比](https://pub-500a3a9e99b44ef29efa70fd87011d69.r2.dev/agent/llm应用开发原理agent经典开发范式/04-comparison-paradigms.png)

| 范式名称                      | 核心逻辑 (Motto)                    | 适用场景 (Best For)                                         | 优点                                                | 缺点 (Cons)                                                  | 开发复杂度 | 成本/延迟 |
| --- | --- | --- | --- | --- | --- | --- |
| ReAct (Reason+Act)            | "边想边做"<br/>(Thought→Act→Obs)    | 通用型<br/>查天气、简单搜索、单次工具调用                   | ✅ 最通用，生态支持最好。<br/>✅ 开发门槛低，上手快。 | ❌ 长链路任务容易"迷路"<br/>❌ 容易陷入死循环                  | ★          | 低        |
| Plan-and-Execute (规划与执行) | "谋定而后动"<br/>(Planner→Executor) | 长链路复杂任务<br/>写长短报告、从零完成项目、多步骤数据分析 | ✅ 目标感强，不易跑偏。<br/>✅ 逻辑清晰，步骤可控。   | ❌ 初始计划可能不准确。<br/>❌ 灵活性稍差（除非加 Re-planning） | ★★         | 中        |
| Reflection (反思/自修正)      | "三省吾身"<br/>(Do→Critique→Fix)    | 高精度任务<br/>写代码(Code Gen)、数学解题、翻译优化         | ✅ 产出质量最高。<br/>✅ 容错率极高，能自动修 Bug。   | ❌ 最慢，延迟高。<br/>❌ Token 消耗翻倍（反复重试）            | ★★         | 高        |
...
> **延伸阅读**：
> - [LangGraph: Stateful Agents for Complex Workflows](https://langchain-ai.github.io/langgraph/) - 组合范式的高级框架
> - [Semantic Kernel: Orchestrate AI Skills](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts-semantic-kernel) - 微软的技能编排框架
> - [AI Agents: A Survey](https://arxiv.org/abs/2310.12861) - Agent架构综合调研
